...
function Page() {
  const { data: smartTransportData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Smart Transport Admin" subtitle="Smart transport administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Smart Transport Admin"
          subtitle="Smart transport administration workspace"
          message="We couldn't load Smart Transport Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
