...
function Page() {
  const { data: zooData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Zoo Operations Manager" subtitle="Zoo operations management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Zoo Operations Manager"
          subtitle="Zoo operations management workspace"
          message="We couldn't load Zoo Operations Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
