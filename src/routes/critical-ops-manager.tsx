...
function Page() {
  const { data: criticalOpsData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Critical Ops Manager" subtitle="Critical operations management" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Critical Ops Manager"
          subtitle="Critical operations management"
          message="We couldn't load Critical Ops Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
