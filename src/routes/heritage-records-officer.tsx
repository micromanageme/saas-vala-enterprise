...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Heritage Records Officer" subtitle="Heritage records management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Heritage Records Officer"
          subtitle="Heritage records management workspace"
          message="We couldn't load Heritage Records Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
