...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Recovery Commander" subtitle="Recovery command center" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Recovery Commander"
          subtitle="Recovery command center"
          message="We couldn't load Recovery Commander data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
