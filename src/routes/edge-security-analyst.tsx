...
function Page() {
  const { data: edgeSecurityData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Edge Security Analyst" subtitle="Edge security analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Edge Security Analyst"
          subtitle="Edge security analysis workspace"
          message="We couldn't load Edge Security Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
