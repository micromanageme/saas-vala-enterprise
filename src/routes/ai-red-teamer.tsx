...
function Page() {
  const { data: redTeamData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="AI Red Teamer" subtitle="AI red teaming workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="AI Red Teamer"
          subtitle="AI red teaming workspace"
          message="We couldn't load AI Red Teamer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
