...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Traffic Routing Engineer" subtitle="Traffic routing engineering workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Traffic Routing Engineer"
          subtitle="Traffic routing engineering workspace"
          message="We couldn't load Traffic Routing Engineer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
