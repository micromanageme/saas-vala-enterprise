...
function Page() {
  const { data: scrumData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Scrum Master" subtitle="Scrum Master workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Scrum Master"
          subtitle="Scrum Master workspace"
          message="We couldn't load Scrum Master data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
