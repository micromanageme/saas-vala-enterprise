...
function Page() {
  const { data: cdnData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="CDN Edge Operator" subtitle="CDN edge operations workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="CDN Edge Operator"
          subtitle="CDN edge operations workspace"
          message="We couldn't load CDN Edge Operator data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
