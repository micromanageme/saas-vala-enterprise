...
function Page() {
  const { data: groundData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Ground Operations Control" subtitle="Ground operations control workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Ground Operations Control"
          subtitle="Ground operations control workspace"
          message="We couldn't load Ground Operations Control data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
