...
function Page() {
  const { data: featureData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Feature Store Admin" subtitle="Feature store administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Feature Store Admin"
          subtitle="Feature store administration workspace"
          message="We couldn't load Feature Store Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
