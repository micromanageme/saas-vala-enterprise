...
function Page() {
  const { data: interoperabilityData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Interoperability Fabric" subtitle="Cross-platform orchestration, protocol translation, legacy-modern bridge" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Interoperability Fabric"
          subtitle="Cross-platform orchestration, protocol translation, legacy-modern bridge"
          message="We couldn't load Root Interoperability Fabric data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
