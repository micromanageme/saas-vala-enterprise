...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Multiverse Environment Admin" subtitle="Multiverse environment administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Multiverse Environment Admin"
          subtitle="Multiverse environment administration workspace"
          message="We couldn't load Multiverse Environment Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
