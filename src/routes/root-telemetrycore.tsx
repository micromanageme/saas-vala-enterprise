...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Universal Telemetry Core" subtitle="Telemetry federation, infinite-scale observability, ultra-high-frequency monitoring" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Universal Telemetry Core"
          subtitle="Telemetry federation, infinite-scale observability, ultra-high-frequency monitoring"
          message="We couldn't load Root Universal Telemetry Core data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
