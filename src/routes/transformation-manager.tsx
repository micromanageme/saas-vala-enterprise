...
function Page() {
  const { data: transformData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Transformation Manager" subtitle="Transformation management workspace" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Transformation management workspace"
          message="We couldn't load Transformation Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
