...
function Page() {
  const { data: tamperData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Tamper Detection Officer" subtitle="Tamper detection workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Tamper Detection Officer"
          subtitle="Tamper detection workspace"
          message="We couldn't load Tamper Detection Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
