...
function Page() {
  const { data: hospitalityData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Customer Hospitality Lead" subtitle="Customer hospitality leadership workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Customer Hospitality Lead"
          subtitle="Customer hospitality leadership workspace"
          message="We couldn't load Customer Hospitality Lead data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
