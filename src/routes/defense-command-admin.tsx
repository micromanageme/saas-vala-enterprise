...
function Page() {
  const { data: defenseData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Defense Command Admin" subtitle="Defense command administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Defense Command Admin"
          subtitle="Defense command administration workspace"
          message="We couldn't load Defense Command Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
