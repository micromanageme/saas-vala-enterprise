...
function Page() {
  const { data: eventBusData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Event Bus Admin" subtitle="Event bus administration" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Event Bus Admin"
          subtitle="Event bus administration"
          message="We couldn't load Event Bus Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
