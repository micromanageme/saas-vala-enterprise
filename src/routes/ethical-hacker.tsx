...
function Page() {
  const { data: hackerData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Ethical Hacker" subtitle="Penetration testing and security assessment" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Penetration testing and security assessment"
          message="We couldn't load Ethical Hacker data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
