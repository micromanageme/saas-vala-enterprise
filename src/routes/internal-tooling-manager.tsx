...
function Page() {
  const { data: toolingData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Internal Tooling Manager" subtitle="Internal tooling management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Internal Tooling Manager"
          subtitle="Internal tooling management workspace"
          message="We couldn't load Internal Tooling Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
