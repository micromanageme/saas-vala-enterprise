...
function Page() {
  const { data: rescueData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Rescue Operations Manager" subtitle="Rescue operations management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Rescue Operations Manager"
          subtitle="Rescue operations management workspace"
          message="We couldn't load Rescue Operations Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
