...
function Page() {
  const { data: facilityData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Facility Manager" subtitle="Facility management workspace" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Facility management workspace"
          message="We couldn't load Facility Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
