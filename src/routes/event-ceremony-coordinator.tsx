...
function Page() {
  const { data: ceremonyData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Event Ceremony Coordinator" subtitle="Event ceremony coordination workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Event Ceremony Coordinator"
          subtitle="Event ceremony coordination workspace"
          message="We couldn't load Event Ceremony Coordinator data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
