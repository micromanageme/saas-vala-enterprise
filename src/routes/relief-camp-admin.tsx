...
function Page() {
  const { data: campData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Relief Camp Admin" subtitle="Relief camp administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Relief Camp Admin"
          subtitle="Relief camp administration workspace"
          message="We couldn't load Relief Camp Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
