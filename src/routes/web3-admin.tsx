...
function Page() {
  const { data: web3Data, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Web3 Admin" subtitle="Web3 administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Web3 Admin"
          subtitle="Web3 administration workspace"
          message="We couldn't load Web3 Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
