...
function Page() {
  const { data: envData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Environmental Manager" subtitle="Environmental management workspace" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Environmental management workspace"
          message="We couldn't load Environmental Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
