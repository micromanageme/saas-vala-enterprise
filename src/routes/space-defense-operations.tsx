...
function Page() {
  const { data: spaceDefenseData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Space Defense Operations" subtitle="Space defense operations workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Space Defense Operations"
          subtitle="Space defense operations workspace"
          message="We couldn't load Space Defense Operations data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
