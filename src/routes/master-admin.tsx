...
function Page() {
  const { data: masterData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Master Admin" subtitle="Master-level administration across all entities" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Master Admin"
          subtitle="Master-level administration across all entities"
          message="We couldn't load Master Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
