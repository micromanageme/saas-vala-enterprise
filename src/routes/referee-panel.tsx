...
function Page() {
  const { data: refereeData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Referee Panel" subtitle="Referee panel workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Referee Panel"
          subtitle="Referee panel workspace"
          message="We couldn't load Referee Panel data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
