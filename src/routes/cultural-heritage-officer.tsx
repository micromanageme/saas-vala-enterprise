...
function Page() {
  const { data: heritageData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Cultural Heritage Officer" subtitle="Cultural heritage workspace" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Cultural heritage workspace"
          message="We couldn't load Cultural Heritage Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
