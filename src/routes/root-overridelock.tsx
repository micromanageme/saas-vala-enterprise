...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Universal Override Lock" subtitle="Override conflict arbitration, emergency override sequencing, irreversible safeguard barriers" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Universal Override Lock"
          subtitle="Override conflict arbitration, emergency override sequencing, irreversible safeguard barriers"
          message="We couldn't load Root Universal Override Lock data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
