...
function Page() {
  const { data: labelingData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Data Labeling Manager" subtitle="Data labeling management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Data Labeling Manager"
          subtitle="Data labeling management workspace"
          message="We couldn't load Data Labeling Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
