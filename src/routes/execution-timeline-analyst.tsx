...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Execution Timeline Analyst" subtitle="Execution timeline analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Execution Timeline Analyst"
          subtitle="Execution timeline analysis workspace"
          message="We couldn't load Execution Timeline Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
