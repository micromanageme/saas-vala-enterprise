...
function Page() {
  const { data: failoverData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Failover Engineer" subtitle="Failover engineering workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Failover Engineer"
          subtitle="Failover engineering workspace"
          message="We couldn't load Failover Engineer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
