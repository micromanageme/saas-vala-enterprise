...
function Page() {
  const { data: carbonData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Carbon Accounting Manager" subtitle="Carbon accounting management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Carbon Accounting Manager"
          subtitle="Carbon accounting management workspace"
          message="We couldn't load Carbon Accounting Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
