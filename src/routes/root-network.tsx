...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Network Operations" subtitle="DNS routing, gateway traffic, proxy management" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Network Operations"
          subtitle="DNS routing, gateway traffic, proxy management"
          message="We couldn't load Root Network Operations data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
