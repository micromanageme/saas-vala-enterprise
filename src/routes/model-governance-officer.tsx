...
function Page() {
  const { data: modelGovData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Model Governance Officer" subtitle="Model governance oversight" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Model Governance Officer"
          subtitle="Model governance oversight"
          message="We couldn't load Model Governance Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
