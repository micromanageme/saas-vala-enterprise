...
function Page() {
  const { data: airDefenseData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Air Defense Operations" subtitle="Air defense operations workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Air Defense Operations"
          subtitle="Air defense operations workspace"
          message="We couldn't load Air Defense Operations data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
