...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Alert Orchestration Engineer" subtitle="Alert orchestration engineering workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Alert Orchestration Engineer"
          subtitle="Alert orchestration engineering workspace"
          message="We couldn't load Alert Orchestration Engineer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
