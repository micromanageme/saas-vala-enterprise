...
function Page() {
  const { data: cropData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Crop Analyst" subtitle="Crop analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Crop Analyst"
          subtitle="Crop analysis workspace"
          message="We couldn't load Crop Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
