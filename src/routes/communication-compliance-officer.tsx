...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Communication Compliance Officer" subtitle="Communication compliance workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Communication Compliance Officer"
          subtitle="Communication compliance workspace"
          message="We couldn't load Communication Compliance Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
