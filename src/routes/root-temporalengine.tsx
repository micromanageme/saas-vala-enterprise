...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Universal Temporal Engine" subtitle="Historical state replay, point-in-time restoration, timeline branching" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Universal Temporal Engine"
          subtitle="Historical state replay, point-in-time restoration, timeline branching"
          message="We couldn't load Universal Temporal Engine data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
