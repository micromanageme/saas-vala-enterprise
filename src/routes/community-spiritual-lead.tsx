...
function Page() {
  const { data: spiritualLeadData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Community Spiritual Lead" subtitle="Community spiritual leadership workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Community Spiritual Lead"
          subtitle="Community spiritual leadership workspace"
          message="We couldn't load Community Spiritual Lead data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
