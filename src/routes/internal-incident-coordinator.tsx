...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Internal Incident Coordinator" subtitle="Internal incident coordination workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Internal Incident Coordinator"
          subtitle="Internal incident coordination workspace"
          message="We couldn't load Internal Incident Coordinator data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
