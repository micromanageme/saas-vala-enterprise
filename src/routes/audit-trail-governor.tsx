...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Audit Trail Governor" subtitle="Audit trail governance workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Audit Trail Governor"
          subtitle="Audit trail governance workspace"
          message="We couldn't load Audit Trail Governor data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
