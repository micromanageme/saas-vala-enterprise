...
function Page() {
  const { data: edgeData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Edge Node Operator" subtitle="Edge node operations workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Edge Node Operator"
          subtitle="Edge node operations workspace"
          message="We couldn't load Edge Node Operator data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
