...
function Page() {
  const { data: privilegedData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Privileged Access Governance Lead" subtitle="Privileged access governance workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Privileged Access Governance Lead"
          subtitle="Privileged access governance workspace"
          message="We couldn't load Privileged Access Governance Lead data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
