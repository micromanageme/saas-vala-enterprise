...
function Page() {
  const { data: drCommanderData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Disaster Recovery Commander" subtitle="Disaster recovery command" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Disaster Recovery Commander"
          subtitle="Disaster recovery command"
          message="We couldn't load Disaster Recovery Commander data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
