...
function Page() {
  const { data: rightsData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Digital Rights Officer" subtitle="Digital rights management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Digital Rights Officer"
          subtitle="Digital rights management workspace"
          message="We couldn't load Digital Rights Officer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
