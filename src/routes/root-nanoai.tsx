...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Nano AI Governance Loop" subtitle="Recursive AI behavior validation, prompt mutation tracing, autonomous drift prevention" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Nano AI Governance Loop"
          subtitle="Recursive AI behavior validation, prompt mutation tracing, autonomous drift prevention"
          message="We couldn't load Nano AI Governance Loop data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
