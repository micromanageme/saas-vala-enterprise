...
function Page() {
  const { data: flagsData, isLoading, error, refetch } = useQuery({
    queryKey: ["root-flags"],
    queryFn: async () => {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Feature Flag Engine" subtitle="Staged rollout, tenant feature controls, emergency disable" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Feature Flag Engine"
          subtitle="Staged rollout, tenant feature controls, emergency disable"
          message="We couldn't load Root Feature Flag Engine data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
