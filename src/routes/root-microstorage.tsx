...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Micro Storage Integrity Engine" subtitle="Write-corruption validation, fragmented state healing, storage entropy monitoring" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Micro Storage Integrity Engine"
          subtitle="Write-corruption validation, fragmented state healing, storage entropy monitoring"
          message="We couldn't load Micro Storage Integrity Engine data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
