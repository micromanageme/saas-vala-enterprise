...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="SLA Escalation Manager" subtitle="SLA escalation management" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="SLA Escalation Manager"
          subtitle="SLA escalation management"
          message="We couldn't load SLA Escalation Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
