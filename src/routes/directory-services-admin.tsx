...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Directory Services Admin" subtitle="Directory services administration workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Directory Services Admin"
          subtitle="Directory services administration workspace"
          message="We couldn't load Directory Services Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
