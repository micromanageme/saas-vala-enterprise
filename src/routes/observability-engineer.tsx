...
function Page() {
  const { data: obsData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Observability Engineer" subtitle="System observability and monitoring" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="System observability and monitoring"
          message="We couldn't load Observability Engineer data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
