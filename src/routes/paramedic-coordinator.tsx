...
function Page() {
  const { data: paramedicData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Paramedic Coordinator" subtitle="Paramedic coordination workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Paramedic Coordinator"
          subtitle="Paramedic coordination workspace"
          message="We couldn't load Paramedic Coordinator data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
