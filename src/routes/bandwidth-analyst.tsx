...
function Page() {
  const { data: bandwidthData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Bandwidth Analyst" subtitle="Bandwidth analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Bandwidth Analyst"
          subtitle="Bandwidth analysis workspace"
          message="We couldn't load Bandwidth Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
