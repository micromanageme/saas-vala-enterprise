...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Root Observability Fabric" subtitle="Unified telemetry, distributed tracing, correlation engine" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Root Observability Fabric"
          subtitle="Unified telemetry, distributed tracing, correlation engine"
          message="We couldn't load Root Observability Fabric data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
