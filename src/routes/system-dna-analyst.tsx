...
function Page() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="System DNA Analyst" subtitle="System DNA analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="System DNA Analyst"
          subtitle="System DNA analysis workspace"
          message="We couldn't load System DNA Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
