...
function Page() {
  const { data: newsroomData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Newsroom Manager" subtitle="Newsroom management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Newsroom Manager"
          subtitle="Newsroom management workspace"
          message="We couldn't load Newsroom Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
