...
function Page() {
  const { data: geneticsData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Genetics Analyst" subtitle="Genetics analysis workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Genetics Analyst"
          subtitle="Genetics analysis workspace"
          message="We couldn't load Genetics Analyst data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
