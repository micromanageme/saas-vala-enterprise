...
function Page() {
  const { data: housekeepingData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Housekeeping Supervisor" subtitle="Housekeeping supervision workspace" />
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <DashboardError
          ...
          subtitle="Housekeeping supervision workspace"
          message="We couldn't load Housekeeping Supervisor data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
