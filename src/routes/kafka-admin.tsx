...
function Page() {
  const { data: kafkaData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Kafka Admin" subtitle="Kafka administration" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Kafka Admin"
          subtitle="Kafka administration"
          message="We couldn't load Kafka Admin data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
