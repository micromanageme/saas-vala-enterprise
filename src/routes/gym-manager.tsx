...
function Page() {
  const { data: gymData, isLoading, error, refetch } = useQuery({
  ...
  if (isLoading) {
    return (
      <AppShell>
        <DashboardSkeleton title="Gym Manager" subtitle="Gym management workspace" />
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <DashboardError
          title="Gym Manager"
          subtitle="Gym management workspace"
          message="We couldn't load Gym Manager data. The service may be unavailable or you may not have permission."
          onRetry={() => refetch()}
        />
      </AppShell>
    );
